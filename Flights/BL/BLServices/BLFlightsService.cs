...
namespace BL.BLServices
{
    public class BLFlightsService : IBLFlights
    {
        ...
        {
            var flights = DalFlights.GetFlightsForWorker(workerId);

            // Assuming you have a Mapper to map Flight entities to FlightDTOs
            var flightDtos = flights.Select(f => mapper.Map<BLFlights>(f)).ToList();
