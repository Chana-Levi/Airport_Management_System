...
namespace DAL.DalServices
{
    ...
    {
        FlightContext _FlightsContext;
        public FlightService(FlightContext flightsContext)
        {
        ...
        //פונקציה חדשה!!
        public List<Flight> GetFlightsForWorker(int workerId)
        {
            return new();
            //return _FlightsContext.Flights
            //    .Include(f => f.Airline)
            //    .Include(f => f.FlightCrews)
            //        .ThenInclude(fc => fc.WorkerId) // Make sure the property name matches your data model
            //    .Include(f => f.Orders)
            //        .ThenInclude(o => o.Passenger)
            //    .Where(f => f.FlightCrews.Any(fc => fc.WorkerId == workerId))
