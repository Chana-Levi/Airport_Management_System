...
namespace Flights.Controllers
{
    ...
    public class FlightsController : ControllerBase
    {
        ...
        #region Get function
        //זה פונקציה חדשה אני צריכהלבדוק טוב שזה נכון
        [HttpGet("worker/{workerId}")]
        public IActionResult GetFlightsForWorker(int workerId)
        {


            var flight = flights.BLGetFlightsForWorker(workerId); // Assuming BLGetFlightsForWorker returns a list of BLFlights
            if (flights == null || flight.Count == 0)
            {
                return NotFound("No flights found for the specified worker.");
            }
